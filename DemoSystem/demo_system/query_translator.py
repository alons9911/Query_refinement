...
def set_fields_to_dict_query(fields: List, dict_query: Dict):
    ...
    for field in fields:
        if field['operator'] in ['in', 'IN', 'In']:
            categorical_conds[field['field']] = json.loads(field['value'])
        else:
            numeric_conds[field['field']] = [field['operator'], float(field['value']), 1]
    return dict_query
...
def translate_dict_query(table_name: str, dict_query: Dict):
    ...
    for attr in selection_numeric_attributes:
        ...
        conds.append(f"{table_char}.{attr} {operator} {value}")
    for attr in selection_categorical_attributes:
        values = categorical_conds[attr]
        conds.append(f"{table_char}.{attr} in {values}")
