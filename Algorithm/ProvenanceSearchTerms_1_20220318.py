...
I tried to define function programmatically on the fly,
but I can't define variables on the fly in that function
    ...
    def get_provenance(row, fc_dic, fc, protected_greater_than):
        ...
        if sensitive_values_of_fc == fairness_value_of_row.to_dict():
            ...
            # for t in selected_attributes:
                ...
                #     provenance_in_one += f" (" + str(terms[t]) + " " + selection_numeric_attributes[t][
                #         0] + f" {" + t + f"})*"
                # else:
                #     provenance_in_one += f""" ({""" + t + "_" + terms[t] + f"})*"
