...
In relaxation, use top-down traversal to find a bounding query
...
def searchPVT(PVT, PVT_head, categorical_att_columns, numeric_attributes, categorical_attributes,
    ...
    # find a bound relaxation by traversing top-down
    def traverse_top_down(row):
        nonlocal find_bounding_relaxation
        nonlocal row_idx
        nonlocal old_bounding_relaxation_location
        nonlocal new_bounding_relaxation_location
        nonlocal old_value_assignment
        nonlocal new_value_assignment
        nonlocal last_satisfying_full_value_assignment
        nonlocal no_need_to_tighten
        nonlocal PVT_head
        if not find_bounding_relaxation:
            nan_row = row.isnull()
            if not nan_row.values.any():
                new_value_assignment = row.tolist()
                new_bounding_relaxation_location = [row_idx] * len(PVT_head)
            else:
                new_value_assignment = copy.deepcopy(old_value_assignment)
                col_id = 0
                for k in nan_row:
                    if not k:
                        new_bounding_relaxation_location[col_id] += 1
                        new_value_assignment[col_id] = row[col_id]
                    col_id += 1
                # optimization: if only one column is moved down, no need to tighten the result
                if sum(k is False for k in nan_row) == 1:
                    no_need_to_tighten = True
            ...
            if assign_to_provenance_relax_only(full_value_assignment, numeric_attributes, categorical_attributes,
                ...
                old_bounding_relaxation_location = new_bounding_relaxation_location
                checked_assignments_not_satisfying.append([full_value_assignment[k] for k in full_PVT_head])
        row_idx += 1

    PVT.apply(traverse_top_down, axis=1)


    # FIXME: recursion PVT may need to be shorter if there are too many nans
    # find a bounding relaxation y binary search
    ...
    #                     col_id += 1
    #                 # optimization: if only one column is moved down, no need to tighten the result
    #                 if sum(k is True for k in nan_row) == 1:
    #                     no_need_to_tighten = True
    #             full_value_assignment = dict(zip(PVT_head, new_value_assignment))
    #             full_value_assignment = {**full_value_assignment, **fixed_value_assignments}
    #             print("value_assignment: ", full_value_assignment)
    ...
    #     row_idx += 1
    #
    # PVT.apply(binary_search, axis=1)
    ...
    def recursion(column):
        ...
        while idx_in_this_col > 0:
            ...
            if len(new_PVT_head) == 1:
                ...
                new_max_index_PVT = [len(PVT_for_recursion)]
            else:
